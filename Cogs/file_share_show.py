...
def file_share_show(ctx, database):
    ...
    if data[1]:
        file_shared = database.select('''SELECT * FROM cantina_administration.file_sharing''', None)
    else:
        file_shared = database.select('''SELECT * FROM cantina_administration.file_sharing WHERE 
